...
public class ProcesaTabla {
    ...
    public void doAll(){
        String consulta = "SELECT * FROM "+tabla+" WHERE ROWNUM = 1";
        try
        {
    ...
    public void setComentarios() throws SQLException{
        comentarios = new String[rsmd.getColumnCount()];
        for (int i = 1; i <= rsmd.getColumnCount(); i++) {
                String comentario = getComentarioCampo(tabla, rsmd.getColumnName(i));
    ...
    private void createFile() throws IOException {
        ...
        if(claseTabla.exists()){
            ...
            for (int i = 0; i < campos.length; i++) {
                pw.println("    /**");
                pw.println("     * Obtiene "+comentarios[i]);
                pw.println("     */");
                ...
                pw.println("                  return "+campos[i]+(tipos[i].equals("String")?".replace(\"'\",\"\")":"")+";");
                if(tipos[i].equals("String")){
                    pw.println("          }");
                }
                pw.println("     }");
                pw.println();
                pw.println("    /**");
                pw.println("     * Establece "+comentarios[i]);
                pw.println("     */");
                pw.println("     public void set"+campos[i]+"("+tipos[i]+" "+campos[i]+"){");
                if(tipos[i].equals("String")){
                    pw.println("          if("+campos[i]+" != null){ "+campos[i]+" = "+campos[i]+".replace(\"'\",\"\"); }");
                }
                pw.println("          this."+campos[i]+" = "+campos[i]+";");
                pw.println("     }");
                pw.println();
    ...
    private String getComentarioCampo(String tabla, String campo) {
        String comentario = "";

        String strSQL = "SELECT * ";
        strSQL = strSQL + " FROM USER_COL_COMMENTS WHERE TABLE_NAME = '" + tabla +"' AND COLUMN_NAME = '"+campo+"'";
        ...
        try
        {
            ...
            ResultSet rs = SentenciaSQL.executeQuery(strSQL);
            if(rs.next()){
                comentario = rs.getString("COMMENTS");
            }
            rs.close();
            SentenciaSQL.close();
        }
        ...
        if(comentario == null || comentario.equalsIgnoreCase("null")){
            if(campo.equals("ESTATUSREGISTRO")){
                comentario = "el estatus actual del registro en la base de datos";
            }
            if(campo.equals("USUARIOCREACION")){
                comentario = "el usuario que creó el registro";
            }
            if(campo.equals("FECHACREACION")){
                comentario = "la fecha en que se creó el registro";
            }
            if(campo.equals("USUARIOMODIFICACION")){
                comentario = "el último usuario que modificó el registro";
            }
            if(campo.equals("FECHAMODIFICACION")){
                comentario = "la fecha en que se modificó el registro por última vez";
            }
            if(campo.equals("OBSREGISTRO")){
                comentario = "las observaciones del registro actual";
            }
        }
