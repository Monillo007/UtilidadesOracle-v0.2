...
public class ModeloLista extends AbstractTableModel{
    ...
    public boolean isCellEditable(int row, int col) {        
        if (col == 0 || col == 1 || col == 2 || col == 3 || ((col==6 || col == 7 || col == 5) && habilitarCamposCombo)) {
            return true;
        } else {
            return false;
        }
...
//                }
                if(tipo.equals("campos")){
                    ...
                    setValueAt("CAT_"+getValueAt(row, 4).toString().substring(2), row, 5);
                    setValueAt(getValueAt(row, 4).toString(), row, 6);
                    setValueAt("DESCRIPCION", row, 7);
                }
            }
        }
        if(col == 0){
